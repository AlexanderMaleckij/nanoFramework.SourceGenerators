...
{
    internal sealed class ResourceContentTypeProvider : IResourceContentTypeProvider
    {
        private const string FallbackContentType = "application/octet-stream";

        public ResourceContentTypeProvider()
        ...
        public ResourceContentTypeProvider(IReadOnlyDictionary<string, string> mapping)
        {
        ...
        public string GetContentType(string resourceName)
        {
            // Retrieve the last two extensions to handle resource names such as "script.js.gz".
            var (penultimateExtension, lastExtension) = PathUtils.GetLastTwoExtensions(resourceName);

            if (penultimateExtension != null)
            {
                return Mappings.TryGetValue(penultimateExtension, out string contentType)
                    ? contentType
                    : FallbackContentType;
            }
            ...
            {
                return Mappings.TryGetValue(lastExtension, out string contentType)
                    ? contentType
                    : FallbackContentType;
            }
